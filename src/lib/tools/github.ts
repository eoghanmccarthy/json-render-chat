...
function handleGitHubError(res: Response, context: string) {
  ...
  if (res.status === 403) return { error: "GitHub API rate limit exceeded. Try again later." };
...
export const getGitHubPullRequests = tool({
  ...
  execute: async ({ owner, repo, state, sort, perPage }) => {
    ...
    const prs = (await res.json()) as Array<GitHubPR>;
    ...
    const enriched = await Promise.all(
      prs.map(async (pr) => {
        ...
        const reviews: Array<GitHubPRReview> = reviewsRes.ok
          ? ((await reviewsRes.json()) as Array<GitHubPRReview>)
          : [];

        let reactionCount = 0;
        if (reactionsRes.ok) {
          const reactions = (await reactionsRes.json()) as Array<GitHubPRReaction>;
