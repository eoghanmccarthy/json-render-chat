...
export const getHackerNewsTop = tool({
  ...
  inputSchema: z.object({
    count: z.number().min(1).max(30).describe("Number of top stories to fetch (1-30)"),
  }),
  execute: async ({ count }) => {
    const topUrl = "https://hacker-news.firebaseio.com/v0/topstories.json?print=pretty";
    ...
    const topIds = (await topRes.json()) as Array<number>;
